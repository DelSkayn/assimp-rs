...
impl Importer {
    ...
    /// Load a scene from a string with a hint.
    /// The hint is a string containing the file extension of the format the data given.
    /// If the library is not able to load with the given format it will still try to determin a
    /// valid format on its own.
    ///
    /// If the call succeeds, return value is `Ok`, containing the loaded `Scene` structure.
    /// If the call fails, return value is `Err`, containing the error string returned from
    /// the Assimp library.
    pub fn read_string_with_hint<'a>(&self, data: &str,hint: &str) -> Result<Scene<'a>, &str> {
        let cstr = CString::new(data).unwrap();
        let hint_cstr = CString::new(hint).unwrap();
        let raw_scene = unsafe {
            aiImportFileFromMemoryWithProperties(
                cstr.as_ptr(),
                data.len() as u32,
